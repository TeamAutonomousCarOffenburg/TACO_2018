...
public class BodyPart implements IBodyPart, Serializable
{
	...
	public BodyPart(IBodyPartConfiguration config)
	{
		...
		if (!config.isRootBody()) {
			joint = new HingeJoint(config.getJointConfiguration());
			joint.updateSensors(flatSensors, structuredSensors);
		}
		...
		if (sensorConfig != null) {
			...
			GyroRate gyroRate = new GyroRate(sensorConfig);
		...
		if (sensorConfig != null) {
			...
			Accelerometer accelerometer = new Accelerometer(sensorConfig);
		...
		if (sensorConfig != null) {
			...
			ForceResistance forceResistance = new ForceResistance(sensorConfig);
		...
		if (sensorConfig != null) {
			...
			Compass compass = new Compass(sensorConfig);
		...
		if (sensorConfig != null) {
			...
			IMU imu = new IMU(sensorConfig);
			imu.updateSensors(flatSensors, structuredSensors);
		}
	}
	...
	public BodyPart(BodyPart source, BodyPart parent)
	{
		...
		if (source.joint != null) {
			...
			this.joint = (HingeJoint) structuredSensors.get(source.joint.getName());
		} else {
			joint = null;
		}
