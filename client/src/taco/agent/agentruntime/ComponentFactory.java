...
import taco.agent.decision.behavior.impl.*;
...
public class ComponentFactory
{
	...
	public IAudiCupWorldModel createWorldModel(IAgentModel agentModel, IScenario scenario, IPose3D startPose)
	{
	...
	public BehaviorMap createBehaviors(IAudiCupThoughtModel thoughtModel, ParameterMap params)
	{
		...
		behaviors.put(new PullOutRight(thoughtModel, behaviors));
		behaviors.put(new PullOutLeft(thoughtModel, behaviors));
		...
		behaviors.put(new EmergencyCar(thoughtModel, behaviors));
		behaviors.put(new Merging(thoughtModel, behaviors));
	...
	protected ParameterMap createSpecificParameters()
	{
		...
		result.put(IBehaviorConstants.MERGING, new FollowLaneParameters());
