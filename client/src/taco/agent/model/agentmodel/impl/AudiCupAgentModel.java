...
public class AudiCupAgentModel extends AgentModel implements IAudiCupAgentModel
{
	...
	public AudiCupAgentModel(IAgentMetaModel metaModel, IAgentIKSolver ikSolver)
	{
		...
		// imu
		for (SensorConfiguration config : meta.getImuConfigs()) {
		...
		for (SensorConfiguration config : meta.getRotationConfigs()) {
		...
		// lidar sensors
		if (meta.getLidarConfigs() != null) {
			for (DistanceSensorConfiguration config : meta.getLidarConfigs()) {
				sensors.put(config.getName(), new Lidar(config));
			}
		}
	...
	@Override
	public ILidar getLidar(LidarPosition position)
	{
		return (ILidar) sensors.get(position.perceptorName);
	}
	...
	{
		// Find Basler camera
		CameraConfiguration basler = null;
		for (CameraConfiguration config : getCarMetaModel().getCameraConfigs()) {
			if (config.getName().equals(PerceptorName.BASLER_CAMERA)) {
				basler = config;
			}
		}
		if (basler != null || getCarMetaModel().getCameraConfigs().length == 2) {
			return (ICameraSensor) sensors.get(PerceptorName.BASLER_CAMERA);
		}
	...
	{
		if (getUltrasonic(UltrasonicPosition.FRONT_CENTER) != null) {
			return getUltrasonic(UltrasonicPosition.FRONT_CENTER).getSensorPose().getX();
		}
		return getLidar(LidarPosition.LIDAR_FRONT).getSensorPose().getX();
	}
}
