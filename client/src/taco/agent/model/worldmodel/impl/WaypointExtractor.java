...
{
	static final Pose3D TURN_RIGHT_OFFSET_BEFORE = new Pose3D(-0.4, 0.13);

	static final Pose3D TURN_RIGHT_OFFSET_AFTER = new Pose3D(0.2, 0.1);
	...
	public static DrivePath extractPath(DriveInstructionManager driveInstructionManager, RuntimeSegment currentSegment)
	{
		...
		while (instructionIndex < driveInstructionManager.getNumberOfInstructions()) {
			...
			runningSegment.switchToSegment(nextSegment, nextLink.getPose().getHorizontalAngle());
		}
		return result;
	}
}
