...
public class EnvironmentManager
{
	...
	private void extractParkingSpaces(StreetMap map)
	{
		for (Segment segment : map) {
			...
			for (int i = 0; i < Direction.values().length; i++) {
				...
				IPose3D pose = parking.getOutOptionByRelativeParkingID(i).getPose();
				...
				pose = new Pose3D(pose.getPosition(), pose.getHorizontalAngle().subtract(Angle.ANGLE_90));
	...
	public ParkingSpace getClosestParkingSpace(IPose3D source)
	{
	...
	public RoadSign updateVisisbleRoadSigns(IPose3D carPose, float globalTime)
	{
