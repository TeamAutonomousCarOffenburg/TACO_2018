...
public interface ILaneMiddleSensor {
	 ...
	 */
	void update(LaneMiddlePerceptor perceptor, float time, IPose3D carPose, List<Obstacle> laneMiddles, StreetMap map);
	 ...
	 */
	IPose3D calculateLateralRepositioning(RuntimeSegment currentSegment, float time, IPose3D carPose, StreetMap map);
	 ...
	 * @return the new pose of the car, null if not possible
	 */
	IPose3D calculateSagittalRepositioning(
	 ...
	 * @param currentSegment the segment we come from
	 * @param currentSegment the segment we drive in
	 * @param carPose the current pose of the car
	 * @param delta the delta by which we require to be inside the segment (in m)
	 * @return the new pose of the car, null if not possible
	 */
	IPose3D calculateLateralRepositioningOnSegmentChange(
			RuntimeSegment previousSegment, RuntimeSegment currentSegment, IPose3D carPose, double delta);

	void calculateAngleOffset(RuntimeSegment currentSegment, IPose3D carPose);
	...
	int getConsecutiveFollowRightLanePerforms();

	List<HistoryEntry> getSensorHistory();
