...
public class LaneMiddle
{
	...
	public void update(LaneMiddlePerceptor perceptor, IPose3D carPose, StreetMap map)
	{
		...
		for (int i = 0; i < lines.length; i++) {
			...
			scanPoints[i] = new ScanPoint(carPose, pixelX[i], position, lines[i]);
		}
	...
	public void takeLaneValuesFromMap(long timestamp, IPose3D carPose, StreetMap map)
	{
		...
		// check plausibility
		if (rightLineMap != null && middleLineMap != null && leftLineMap != null) {
			double delta1 = Vector2D.distance(rightLineMap, middleLineMap);
			double delta2 = Vector2D.distance(middleLineMap, leftLineMap);
			if (Math.abs(delta1 - delta2) > 0.2) {
				// not plausible
				middleLineMap = null;
			}
		}
		...
		Vector2D lane = null;
		if (rightLineMap != null && middleLineMap != null) {
			Vector2D direction = rightLineMap.subtract(middleLineMap);
			lane = middleLineMap.add(0.5, direction);
		} else if (middleLineMap != null && leftLineMap != null) {
			Vector2D direction = middleLineMap.subtract(leftLineMap);
			lane = middleLineMap.add(0.5, direction);
		}

		Vector2D rightLine = rightLineMap;
		Vector2D middleLine = middleLineMap;
		Vector2D leftLine = leftLineMap;
		Vector2D laneMiddle = lane;

		int wanted = 586;
		int midX = wanted;
		if (laneMiddle != null) {
			midX = wanted - LaneMiddle.cameraToPixel(laneMiddle.getY());
		} else {
			valid = false;
			confidence = 0;
		}
		...
		int rightLineX = -1;
		if (rightLine != null) {
			rightLineX = wanted - LaneMiddle.cameraToPixel(rightLine.getY());
		}

		int middleLineX = -1;
		if (middleLine != null) {
			middleLineX = wanted - LaneMiddle.cameraToPixel(middleLine.getY());
		}

		int leftLineX = -1;
		if (leftLine != null) {
			leftLineX = wanted - LaneMiddle.cameraToPixel(leftLine.getY());
		}

		update(new LaneMiddlePerceptor(
					   timestamp, midX, wanted, valid, (float) confidence, rightLineX, middleLineX, leftLineX),
				carPose, map);
	}
