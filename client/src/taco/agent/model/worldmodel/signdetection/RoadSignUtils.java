...
public class RoadSignUtils
{
	...
	public static RoadSign findClosestRoadSign(List<RoadSign> allSigns, Pose3D carPose)
	{
			...
			StreetMap map, List<RoadSign> signs, RoadSign sign, float globalTime)
	{
		...
		// detection
		Direction direction = Direction.getDirection(sign.getPose().getHorizontalAngle());
	...
	{
		Direction direction = Direction.getDirection(removedSign.getPose().getHorizontalAngle());
	 ...
	 */
	public static boolean isInVisibleArea(IPose3D globalCameraPose, RoadSign sign)
	{
		Angle angleToSign = globalCameraPose.getHorizontalAngleTo(sign.getPose());
		Direction carDir = Direction.getDirection(globalCameraPose.getHorizontalAngle());
		Direction signDir = Direction.getDirection(sign.getPose().getHorizontalAngle());
	...
	public static Polygon getvisibleArea(IPose3D globalCameraPose)
	{
		...
		Vector2D globalLeftLimit = VectorUtils.to2D(globalCameraPose.applyTo(VectorUtils.to3D(leftLimit)));
		Vector2D globalRightLimit = VectorUtils.to2D(globalCameraPose.applyTo(VectorUtils.to3D(rightLimit)));

		// TODO 3D not proper 3D as of now
	...
	{
		private Pose3D currentCarPose;

		public RoadSignComparator(Pose3D currentCarPose)
		{
