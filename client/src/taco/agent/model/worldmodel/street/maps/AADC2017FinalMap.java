...
public class AADC2017FinalMap
{
	...
	{
		Segment.nextID = 0;
		Segment rootSegment = NonCrossingSegment.createInitialSegment(new Pose3D(2, 0.5), 1, Angle.ZERO);
		...
		MapUtils.addSignsByXML(map, "config/AADC2017_Final_Roadsigns.xml");
		...
		// create sectors
		map.addSector(
				new Sector(86, map.getSegment(86).getOutOption(SOUTH).getPose().applyTo(new Pose3D(-0.1, 0.5, 0))));
