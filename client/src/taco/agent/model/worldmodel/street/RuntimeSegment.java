...
public class RuntimeSegment
{
	...
	public RuntimeSegment(Segment decoratee, Angle globalAngle)
	{
		...
		driveInstruction = DriveInstruction.FOLLOW_LANE;
	}
	...
	public RuntimeSegment(RuntimeSegment currentSegment)
	{
		...
		this.driveInstruction = currentSegment.driveInstruction;
	}
	...
	public void switchToSegment(Segment nextSegment, Angle carAngle)
	{
		...
		if (inLink == null) {
			...
			if (Math.abs(inLink.getPose().getHorizontalAngle().subtract(carAngle).degrees()) > 90) {
	...
	{
		driveInstruction = updateLocal(previousManeuver, currentManeuver);
		return driveInstruction;
	}

	private DriveInstruction updateLocal(Maneuver previousManeuver, Maneuver currentManeuver)
	{
		...
		case MERGE_LEFT:
			if (decoratee.getType() == SegmentType.MERGING_MIDDLE) {
				SegmentLink leftOption = getStraightOption();
				intendedOption = getClosestLinkToPose(leftOption.getPose().applyTo(new Pose3D(1., 0., 0.)));
				consumesDriveInstruction = true;
				return DriveInstruction.MERGE_LEFT;
			}
		}
		...
		} else if (decoratee.getType() == SegmentType.MERGING_MIDDLE) {
			SegmentLink leftOption = getStraightOption();
			intendedOption = getClosestLinkToPose(leftOption.getPose().applyTo(new Pose3D(1., 0., 0.)));
			result = DriveInstruction.FOLLOW_LANE;

		} else if (decoratee.getType() == SegmentType.MERGING_END) {
			intendedOption = getLeftOption();
			result = DriveInstruction.FOLLOW_LANE;
		} else if (decoratee.isMerging()) {
			intendedOption = getStraightOption();
			result = DriveInstruction.FOLLOW_LANE;
