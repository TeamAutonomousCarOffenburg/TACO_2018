...
public class Segment
{
	...
	{
		return getRoadSigns(direction, true);
	}

	/**
	 * @param direction the (incoming) direction for which to get the road signs
	 * @param printError print an error message on the console
	 * @return all road signs attached to the specified in link
	 */
	public List<RoadSign> getRoadSigns(Direction direction, Boolean printError)
	{
	...
	public Vector3D getPosition()
	{
		if (hasInOption(Direction.SOUTH)) {
			...
			return new Vector3D(
					pose.getX() - CROSSING_LENGTH, pose.getY() - CROSSING_HALF_LENGTH - LANE_HALF_WIDTH, pose.getZ());
		} else { // WEST
			IPose3D pose = getInOption(Direction.WEST).getPose();
			return new Vector3D(
					pose.getX() - CROSSING_HALF_LENGTH + LANE_HALF_WIDTH, pose.getY() - CROSSING_LENGTH, pose.getZ());
		}
	}
	...
	public boolean contains(IPose3D pose)
	{
	...
	/**
	 * Checks if the passed position is within this segment or within a segment connected to this
	 * @param position the position in world coordinates
	 * @return true if the passed position is within this or connected segments
	 */
	public boolean isConnectedTo(Vector3D position)
	{
		if (contains(position)) {
			return true;
		}
		for (SegmentLink current : outOptions) {
			if (current != null) {
				Segment nextSegment = current.getSegmentAfter();
				if (nextSegment != null) {
					if (nextSegment.contains(position)) {
						return true;
					}
				}
			}
	...
	public SegmentLink getClosestLinkToPose(IPose3D pose)
	{
		...
		for (Direction current : Direction.values()) {
			double distance;
			if (hasOutOption(current)) {
				distance = Vector3D.distance(getOutOption(current).getPose().getPosition(), pose.getPosition());
			...
			if (hasInOption(current)) {
				distance = Vector3D.distance(getInOption(current).getPose().getPosition(), pose.getPosition());
				if (distance < maxDistance) {
					maxDistance = distance;
	...
	public Segment getNextOrCurrentSegmentFromPose(SegmentLink inLink, IPose3D pose)
	{
		...
		IPose3D beginPose = inLink.getPose();
		...
		// this
		IPose3D middleOfCrossingLocal = new Pose3D(halfLength, ISegment.LANE_HALF_WIDTH);
		IPose3D poseMiddleCrossing = beginPose.applyTo(middleOfCrossingLocal);
		IPose3D poseRelativeToMiddle = poseMiddleCrossing.applyInverseTo(pose);
		...
		// TODO 3D
		Angle toPose = new Pose3D().getHorizontalAngleTo(poseRelativeToMiddle);

		Direction dir = Direction.getDirection(toPose.add(beginPose.getHorizontalAngle()));
		if (hasOutOption(dir)) {
	 ...
	 */
	public static IPose3D getInPoseFromCorrespondingOutPose(IPose3D outPose)
	{
		Pose3D localInPose = new Pose3D(0, ISegment.LANE_WIDTH, Angle.ANGLE_180);
