...
public class DriveSlalom extends AudiCupComplexBehavior
{
	...
	private IPose3D nextCrossing;

	private IPose3D nextDriveTo;
	...
	protected String decideNextBasicBehavior()
	{
		...
		Angle carAngle = getWorldModel().getThisCar().getPose().getHorizontalAngle();
		...
		IPose3D nextTarget = getNextTarget();
	...
	private boolean isBehindUs(IPose3D pose)
	{
		if (pose == null) {
			return false;
		}
	...
	private void determineNextPoints(List<Obstacle> closestObstacles)
	{
		...
		IPose3D potentialDriveTo =
				new Pose3D(obstaclePos.add(crossingDirection, new Vector3D(0, 0.4, 0)), directionAngle);
		...
		if (phase == Phase.CROSS && previousObstacle != null) {
			...
			nextCrossing = new Pose3D(previousObstaclePos.add(0.5, closestObstaclePos.subtract(previousObstaclePos)),
