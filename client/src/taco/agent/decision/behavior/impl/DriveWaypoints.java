...
public class DriveWaypoints extends AudiCupComplexBehavior
{
	...
	protected String decideNextBasicBehavior()
	{
		...
		if (path.size() < 3) {
			...
			path.add(new DrivePoint(nextPoint.getGoalLink(), new Pose3D(),
	...
	private String getSubBehavior(DrivePoint nextPoint)
	{
		...
		case MERGING:
			return IBehaviorConstants.MERGING;
	...
	private String drive(DrivePoint previousPoint, DrivePoint nextPoint, DrivePoint pointAfterNext)
	{
		...
		IPose3D scanLinePose =
				getWorldModel().getThisCar().getPose().applyTo(new Pose3D(LaneMiddle.SCANLINE_DISTANCE, 0));
		...
		case STRAIGHT_FOREVER:
			...
			if (currentSegment.isMergingOut()) {
				return IBehaviorConstants.MERGING;
			} else if (FOLLOW_LANE_ENABLED && laneMiddleReliable && !willTurn(previousPoint, nextPoint)) {
