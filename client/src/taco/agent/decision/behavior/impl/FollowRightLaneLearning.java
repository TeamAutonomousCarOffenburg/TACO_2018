...
public class FollowRightLaneLearning extends FollowRightLane
{
	...
	public FollowRightLaneLearning(IThoughtModel thoughtModel)
	{
		...
		controller.setInputFactor(inputFactor);
	...
	public void perform()
	{
		...
		Vector3D middle = VectorUtils.to3D(SegmentUtils.getLinePositions(currentPose, measurementLine, map)[1]);
		double distance = 1.5;
		if (middle != null) {
			distance = Vector3D.distance(new Vector3D(0.18, 0.225, 0), middle);
			Vector3D globalStart = currentPose.applyTo(new Vector3D(0.18, 0.225, 0));
			Vector3D globalEnd = currentPose.applyTo(middle);
			getThoughtModel().getDrawings().draw("followline" + drawCounter, Color.RED,
					new SubLine(VectorUtils.to2D(globalStart), VectorUtils.to2D(globalEnd), 0.0001));
		}
