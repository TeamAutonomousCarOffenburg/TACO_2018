...
public class GiveWay extends AudiCupBehavior
{
	...
	private static final double MAX_STEERING_ANGLE = 4.;
		...
		case CHECKING_ACTION:
			...
			if (getWorldModel().isCloseToCrosswalk()) {
				if (isPedestrianNearCrosswalk()) {
					...
					// Calculate steering angle based on map because lane detection tends to detect crosswalk lines as
					// street lines
					SegmentLink inLink = getWorldModel().getCurrentSegment().getInLink();
					Angle deltaAngle = inLink.getPose().getHorizontalAngle().subtract(
							getWorldModel().getThisCar().getPose().getHorizontalAngle());
					Angle steeringAngle = deltaAngle;
					if (steeringAngle.degrees() > MAX_STEERING_ANGLE) {
						steeringAngle = Angle.deg(MAX_STEERING_ANGLE);
					} else if (steeringAngle.degrees() < -MAX_STEERING_ANGLE) {
						steeringAngle = Angle.deg(-MAX_STEERING_ANGLE);
					}
