...
public class DriveGeometry
{
			...
			IPose2D currentPose, IPose2D nextWaypoint, double axleSpacing, Angle minAngle, Angle maxAngle)
	{
		...
		Vector2D virtualPoint = currentPose.getPosition().add(
				AngleUtils.getVectorFromAngle(currentPose.getAngle().radians(), CAR_POSE_ADVANCE));
	...
	private Angle getBackwardsSteeringAngle(IPose2D currentPose, IPose2D backwardsPoint, Angle minAngle, Angle maxAngle)
	{
		...
		Vector2D virtualPoint =
				currentPose.getPosition().subtract(AngleUtils.getVectorFromAngle(backwardsAngle.radians(), 0.15));
	...
	{
		return getPoseDotProduct(currentPose, nextPose, axleSpacing);
	}
	...
	public static double getPoseDotProduct(IPose2D currentPose, IPose2D nextPose, double axleSpacing)
	{
		...
		Vector2D subtract = frontAxleLine.getEnd().subtract(currentPose.getPosition());
		if (subtract.getNorm() < 0.0001) {
			return 1;
		}
		Vector2D carVector = subtract.normalize();

		Vector2D waypointVector = nextPose.getPosition().subtract(currentPose.getPosition());
