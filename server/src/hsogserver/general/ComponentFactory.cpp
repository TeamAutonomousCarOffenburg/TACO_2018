...
ICarMetaModel::Ptr ComponentFactory::createCarMetaModel() const
{
	...
	std::string configJson = "CarMetaModel2018.json";
	...
	return boost::make_shared<JSONCarMetaModel>(handler.floorHeight, handler.steeringServoName, handler.mainMotorName,
			...
			handler.infraRedConfigs, handler.ultrasonicConfigs, handler.lidarConfigs, handler.rotationConfigs,
			handler.voltageConfigs, handler.cameraConfigs, handler.servoDriveConfigs, handler.motorConfigs,
...
#if TACO_CONFIG == TACO_2017 || TACO_CONFIG == TACO_2018
