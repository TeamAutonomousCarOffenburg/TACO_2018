...
struct JSONCarMetaModelHandler
{
	...
	// TODO: the lidar should be replaced with a correct vector config that contains 180 DistanceSensorConfigs?!
	std::vector<IDistanceSensorConfig::ConstPtr> lidarConfigs;
	...
	{
		if (keys.size() > 0) {
			if (keys.back().compare("ultrasonicConfigs") == 0 || keys.back().compare("lidarConfigs") == 0) {
			...
			if (std::string("ultrasonicConfigs").compare(type) == 0) {
				ultrasonicConfigs.push_back(boost::dynamic_pointer_cast<const IDistanceSensorConfig>(
						boost::dynamic_pointer_cast<ConfigFactory<taco::ISensorConfig::ConstPtr>>(currentfac)
								->getConfig()));
				return true;
			} else if (std::string("lidarConfigs").compare(type) == 0) {
				lidarConfigs.push_back(boost::dynamic_pointer_cast<const IDistanceSensorConfig>(
						boost::dynamic_pointer_cast<ConfigFactory<taco::ISensorConfig::ConstPtr>>(currentfac)
								->getConfig()));
				return true;
