...
Communication::Communication(
		ICarMetaModel::Ptr carMetaModel, IAction::Ptr action, IPerception::Ptr perception, uint16_t port)
{
	EnvironmentConfiguration::Ptr environmentConfig = boost::make_shared<EnvironmentConfiguration>();
	this->encoder = new PackageEncoder(perception, carMetaModel, environmentConfig);
...
bool Communication::isClientConnected()
{
	return clientFD != -1;
}
bool Communication::isClientDisconnected()
{
