...
{
	currentPoint = 0;
	return findNextDoubleLinePixel(mat, 0, maxWidth);
}

cv::Point ScanLine::findNextDoubleLinePixel(cv::Mat &mat, int minDistance, int maxWidth)
{
	int startX = currentPoint;
	cv::Point p1 = findNextPixel(mat);
	if (p1.x < 0) {
	...
	int i2 = currentPoint;
	while (p2.x >= 0) {
		int delta1 = abs(i1 - startX);
		int delta2 = abs(i2 - i1);
		if (delta1 > minDistance && delta2 > 1 && delta2 < maxWidth) {
			// found a valid line
			return p1;
		}
		p1 = p2;
